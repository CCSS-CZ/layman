/*
 * Note that this control will most likely remain as an example, and not as a core Ext4 form
 * control.  However, the API will be changing in a future release and so should not yet be
 * treated as a final, stable API at this time.
 */

/**
 * A control that allows selection of between two HSRS.LayerManager.PublishForm.MultiSelect controls.
 */
Ext4.define('HSRS.LayerManager.PublishForm.ItemSelector', {
    extend: 'HSRS.LayerManager.PublishForm.MultiSelect',
    alias: ['widget.itemselectorfield', 'widget.itemselector'],
    alternateClassName: ['Ext4.ux.ItemSelector'],
    requires: [
        'Ext4.button.Button',
        'HSRS.LayerManager.PublishForm.MultiSelect'
    ],
    ...
    createList: function(title){
        ...
        return Ext4.create('HSRS.LayerManager.PublishForm.MultiSelect', {
    ...
    createButtons: function() {
        ...
        if (!me.hideNavIcons) {
            Ext4.Array.forEach(me.buttons, function(name) {
                buttons.push({
                    xtype: 'button',
                    tooltip: me.buttonsText[name],
                    handler: me['on' + Ext4.String.capitalize(name) + 'BtnClick'],
                    cls: Ext4.baseCSSPrefix + 'form-itemselector-btn',
                    iconCls: Ext4.baseCSSPrefix + 'form-itemselector-' + name,
                    navBtn: true,
                    scope: me,
                    margin: '4 0 0 0'
                });
     ...
     * @param {Ext4.view.BoundList} list The list to read selections from.
     * @return {Ext4.data.Model[]} The selected records in store order.
     * 
     */
    getSelections: function(list) {
        var store = list.getStore();

        return Ext4.Array.sort(list.getSelectionModel().getSelection(), function(a, b) {
            a = store.indexOf(a);
            b = store.indexOf(b);
    ...
    setValue: function(value) {
        ...
        Ext4.Array.forEach(selected, function(rec){
    ...
    onEnable: function(){
        ...
        Ext4.Array.forEach(me.query('[navBtn]'), function(btn){
            btn.enable();
        });
    },
